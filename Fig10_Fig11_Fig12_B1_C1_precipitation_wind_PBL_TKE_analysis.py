...
varlist_SGAR = [ "T", "APRL","QD", "U10", "V10", "WIND10", "GHPBL"]
var_num_list_SGAR = [ "130", "142", "133", "165", "166", "171", "271"]

varlist_GAR =       [ "T","APRL","APRC","QD", "U10", "V10", "WIND10", "GHPBL"]
var_num_list_GAR = [ "130", "142","143","133", "165", "166", "171", "271"]
...
ax2 = fig.add_subplot(4, 2, 2, projection=rotated_pole)
...
ax3 = fig.add_subplot(4, 2, 5, projection=rotated_pole)
...
ax4 = fig.add_subplot(4, 2, 6, projection=rotated_pole)
...
ax5 = fig.add_subplot(4, 2, 3, projection=rotated_pole)
...
ax6 = fig.add_subplot(4, 2, 4, projection=rotated_pole)
ax6.set_title('')
...
#PBL
ax7 = fig.add_subplot(4, 2, 7, projection=rotated_pole)
rotplot = plot_rotvar(fig, diff_pblh_GAR, ax7, cax3, 'mm', 'Δ PBL [m]', 'RdBu_r', levels_pbl, ticks_pbl, 'both', 'vertical')
ax7.set_title('')
...
ax8 = fig.add_subplot(4, 2, 8, projection=rotated_pole)
rotplot = plot_rotvar(fig, diff_pblh_SGAR , ax8, cax3, 'mm', 'Δ PBL [m]', 'RdBu_r', levels_pbl, ticks_pbl, 'both', 'vertical')
ax8.set_title('')
...
#plt.tight_layout 
plt.savefig(str(dir_out)+'/precip_wind_vectors_transparent_pbl.png',dpi=300, bbox_inches='tight')
...
############################ PLOT FIGURE 10 and 11 #############################
# diff day
plot_diff_11_list   = [diff_pbl_GAR_day,  diff_tke_GAR_day]
plot_diff_0275_list = [diff_pbl_SGAR_day, diff_tke_SGAR_day]
plot_diffdiff_list  = [diffdiff_pbl_day, diffdiff_tke_day]
title_list          = ['PBL, day', 'TKE, day']
...
levels_pbl = list(filter(lambda num: num != 0,  np.arange(-450,500,50)))
ticks_pbl =  np.arange(-400,500, 100)

levels_tke = list(filter(lambda num: num != 0,  np.arange(-0.5,0.55,0.05)))
ticks_tke =  np.arange(-0.5,0.6, 0.1).round(2)

levels_diff_pbl = list(filter(lambda num: num != 0,  np.arange(-100,120,20)))
ticks_diff_pbl  =  np.arange(-100,200, 100)

levels_diff_tke = list(filter(lambda num: num != 0,  np.arange(-0.3,0.35,0.05).round(2)))
ticks_diff_tke  =  np.arange(-0.3,0.4, 0.1).round(2)

levels_list = [levels_pbl, levels_tke]
ticks_list = [ticks_pbl, ticks_tke]

levels_diff_list = [levels_diff_pbl, levels_diff_tke]
ticks_diff_list = [ticks_diff_pbl, ticks_diff_tke]
label_list = ['Δ PBL [m]', 'Δ TKE [Jkg$^{-1}$]']
...
plt.savefig(dir_out+'/pbl_tke_diff_day_spatial_new.png', dpi=300, bbox_inches='tight')

# plot as subplot 
# diff night

plot_diff_11_list   = [diff_pbl_GAR_night,  diff_tke_GAR_night]
plot_diff_0275_list = [diff_pbl_SGAR_night, diff_tke_SGAR_night]
plot_diffdiff_list  = [diffdiff_pbl_night, diffdiff_tke_night]
title_list          = ['PBL, night', 'TKE, night']
...
plt.subplots_adjust(hspace = 0.7)
plt.savefig(dir_out+'/pbl_tke_diff_night_spatial_new.png', dpi=300, bbox_inches='tight')
...
rotplot = plot_rotvar(fig, pbl_night_GAR_noirri, ax5, cax1, 'mm', 'PBL [m]', cmap, levels_pbl, ticks_pbl, 'max', 'horizontal')
ax5.set_title('')
...
rotplot = plot_rotvar(fig, pbl_night_SGAR_irri, ax6, cax1, 'mm', 'PBL [m]', cmap, levels_pbl, ticks_pbl, 'max', 'horizontal')
ax6.set_title('')
...
rotplot = plot_rotvar(fig, pbl_night_GAR_noirri, ax7, cax1, 'mm', 'PBL [m]', cmap, levels_pbl, ticks_pbl, 'max', 'horizontal')
ax7.set_title('')
...
ax9.text(0.0, 1.08, '(a)', transform=ax9.transAxes, fontsize=14)
...
ax10.text(0.0, 1.08, '(b)', transform=ax10.transAxes, fontsize=14)
...
ax11.text(0.0, 1.08, '(c)', transform=ax11.transAxes, fontsize=14)
...
ax12.text(0.0, 1.08, '(d)', transform=ax12.transAxes, fontsize=14)

# PBL night
ax13 = fig.add_subplot(4, 4,13, projection=rotated_pole)
rotplot = plot_rotvar(fig, tke_night_GAR_noirri[48], ax13, cax2, 'mm','TKE [Jkg$^{-1}$]', cmap, levels_tke, ticks_tke, 'max', 'horizontal')
ax13.set_title('')
...
ax13.text(0.0, 1.08, '(e)', transform=ax13.transAxes, fontsize=14)


ax14 = fig.add_subplot(4, 4, 14, projection=rotated_pole)
rotplot = plot_rotvar(fig, tke_night_SGAR_irri[48], ax14, cax2, 'mm', 'TKE [Jkg$^{-1}$]', cmap, levels_tke, ticks_tke, 'max', 'horizontal')
ax14.set_title('')
...
ax14.text(0.0, 1.08, '(f)', transform=ax14.transAxes, fontsize=14)

ax15 = fig.add_subplot(4, 4, 15, projection=rotated_pole)
rotplot = plot_rotvar(fig, tke_night_GAR_noirri[48], ax15, cax2, 'mm', 'TKE [Jkg$^{-1}$]', cmap, levels_tke, ticks_tke, 'max', 'horizontal')
ax15.set_title('')
...
ax15.text(0.0, 1.08, '(g)', transform=ax15.transAxes, fontsize=14)
...
ax16.text(0.0, 1.08, '(h)', transform=ax16.transAxes, fontsize=14)
