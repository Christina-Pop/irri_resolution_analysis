...
def idw_for_model(rlat_source, rlon_source, rlat_target, rlon_target, data, column_name):
    ...
    w = 1.0 / d**2
    precip_idw = np.sum(w * data_source2d[inds], axis=1) / np.sum(w, axis=1)

    df_inttemp = pd.DataFrame()
    #df_inttemp["rlon"] = rlon_target
    #df_inttemp["rlat"] = rlat_target
    df_inttemp[column_name] = np.round(precip_idw, 2)
